...
public class IntegerValue implements Value, INumber {
	...
	public Value unaryOperation(UnaryOperationType operation, Interpreter interpreter, Sequence sequence) {
		return operation == UnaryOperationType.NEGATE ? get(-value) : operation == UnaryOperationType.LOGICAL_NOT ? get(~value) : Value.super.unaryOperation(operation, interpreter, sequence);
	}
	...
	public Value binaryOperation(BinaryOperationType operation, Value right, Interpreter interpreter, Sequence sequence) {
		if(right instanceof INumber num) {
			if(num.numberType().ordinal() > INumber.NumberType.INTEGER.ordinal())
			...
			switch (operation) {
				...
				case DIVIDE:
					...
					return get(value / right_int);
				case EXPONANT:
					return get((int) Math.pow(value, right_int));
				case MODULO:
					if(right_int == 0)
						throw new TError(new DivisionByZeroError(sequence));
					return get(value % right_int);
	...
	@Override
	public NumberType numberType() { return NumberType.INTEGER; }
	...
	public Value handle(INumber left, BinaryOperationType operation, Interpreter interpreter, Sequence sequence) {
		...
		switch (operation) {
			...
			case DIVIDE:
				...
				return get(left_int / value);
			case EXPONANT:
				return get((int) Math.pow(left_int, value));
			case MODULO:
				if(value == 0)
					throw new TError(new DivisionByZeroError(sequence));
				return get(left_int % value);
		...
		throw new TError(new IllegalOperationError(operation.toString(), "integer", sequence));
	}
