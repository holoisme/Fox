...
public class ShortValue implements Value, INumber {
	...
	public Value unaryOperation(UnaryOperationType operation, Interpreter interpreter, Sequence sequence) {
		return operation == UnaryOperationType.NEGATE ? new ShortValue((short) -value) : operation == UnaryOperationType.LOGICAL_NOT ? new ShortValue((short) ~value) : Value.super.unaryOperation(operation, interpreter, sequence);
	}
	...
	public Value binaryOperation(BinaryOperationType operation, Value right, Interpreter interpreter, Sequence sequence) {
		if(right instanceof INumber num) {
			if(num.numberType().ordinal() > INumber.NumberType.SHORT.ordinal())
	...
	public Value quickOperation(QuickOperationType operation, Interpreter interpreter, Sequence sequence) {
		if(operation == QuickOperationType.PLUS_PLUS)
			return IntegerValue.get(value + 1);
	...
	public boolean equalTo(Value other) {
		if(other instanceof ShortValue iv)
	...
	@Override
	public Value typeOf() { return new StringValue("short"); }
	...
	@Override
	public Class<?> toJavaClass(Object selfObject) { return short.class; }

	@Override
	public NumberType numberType() { return NumberType.SHORT; }
	...
	public Value handle(INumber left, BinaryOperationType operation, Interpreter interpreter, Sequence sequence) {
		...
		throw new TError(new IllegalOperationError(operation.toString(), "short", sequence));
	}
